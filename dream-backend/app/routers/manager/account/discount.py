...
def 복지몰할인정산_필터조건 (request: Request):
...
# /be/manager/account/discount/init
@router.post("/manager/account/discount/init", dependencies=[Depends(api_same_origin)])
async def 복지몰할인정산_내역_init (
     request: Request
    ...
    result.update({"filter": 복지몰할인정산_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/discount/list
@router.post("/manager/account/discount/list", dependencies=[Depends(api_same_origin)])
async def 복지몰할인정산_내역 (
     request: Request
