...
def 개인별정산_필터조건 (request: Request):
...
# /be/manager/account/person/init
@router.post("/manager/account/person/init", dependencies=[Depends(api_same_origin)])
async def 개인별정산_내역_init (
     request: Request
    ...
    result.update({"filter": 개인별정산_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/person/list
@router.post("/manager/account/person/list", dependencies=[Depends(api_same_origin)])
async def 개인별정산_내역 (
     request: Request
