...
def 복지카드정산_필터조건 (request: Request):
...
# /be/manager/account/card/init
@router.post("/manager/account/card/init", dependencies=[Depends(api_same_origin)])
async def 복지카드정산_내역_init (
     request: Request
    ...
    result.update({"filter": 복지카드정산_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/card/list
@router.post("/manager/account/card/list", dependencies=[Depends(api_same_origin)])
async def 복지카드정산_내역 (
     request: Request
