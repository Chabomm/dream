...
def 현금영수증_필터조건 (request: Request):
...
# /be/manager/account/receipt/init
@router.post("/manager/account/receipt/init", dependencies=[Depends(api_same_origin)])
async def 현금영수증_내역_init (
     request: Request
    ...
    result.update({"filter": 현금영수증_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/receipt/list
@router.post("/manager/account/receipt/list", dependencies=[Depends(api_same_origin)])
async def 현금영수증_내역 (
     request: Request
