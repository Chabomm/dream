...
def 전체사용정산_필터조건 (request: Request):
...
# /be/manager/account/merge/init
@router.post("/manager/account/merge/init", dependencies=[Depends(api_same_origin)])
async def 전체사용정산_내역_init (
     request: Request
    ...
    result.update({"filter": 전체사용정산_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/merge/list
@router.post("/manager/account/merge/list", dependencies=[Depends(api_same_origin)])
async def 전체사용정산_내역 (
     request: Request
