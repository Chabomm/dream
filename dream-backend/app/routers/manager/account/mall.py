...
def 복지몰사용정산_필터조건 (request: Request):
...
# /be/manager/account/mall/init
@router.post("/manager/account/mall/init", dependencies=[Depends(api_same_origin)])
async def 복지몰사용정산_내역_init (
     request: Request
    ...
    result.update({"filter": 복지몰사용정산_필터조건(request)}) # 초기 필터

    return result

# /be/manager/account/mall/list
@router.post("/manager/account/mall/list", dependencies=[Depends(api_same_origin)])
async def 복지몰사용정산_내역 (
     request: Request
