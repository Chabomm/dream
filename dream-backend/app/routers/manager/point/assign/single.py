...
from app.service.manager.point import single_service
...
def 개별_포인트지급_필터조건 (request: Request):
...
# /be/manager/point/assign/single/init
@router.post("/manager/point/assign/single/init", dependencies=[Depends(api_same_origin)])
async def 개별_포인트지급내역_init (
     request: Request
    ...
    result.update({"filter": 개별_포인트지급_필터조건(request)}) # 초기 필터

    return result

# /be/manager/point/assign/single/list
@router.post("/manager/point/assign/single/list", dependencies=[Depends(api_same_origin)])
async def 개별_포인트_지급내역 (
     request: Request
    ...
    res = single_service.single_list(request, pPage_param)
    request.state.inspect = frame()

    return res


# /be/manager/point/assign/single/read
@router.post("/manager/point/assign/single/read", dependencies=[Depends(api_same_origin)])
async def 개별_포인트_지급내역_상세(
     request: Request
    ...
    res = single_service.single_select_user_list(request, assignSingleRead)
    request.state.inspect = frame()
...
# /be/manager/point/assign/single/edit
@router.post("/manager/point/assign/single/edit", dependencies=[Depends(api_same_origin)])
async def 개별_포인트_지급_회수_등록(
     request: Request
    ...
    if assignSingleInput.save_type == "2" or assignSingleInput.save_type == "1" : # 개별지급 / 대량지급
        ...
        res = single_service.single_point_create(request, assignSingleInput)
        request.state.inspect = frame()
        if res is None :
            return ex.ReturnOK(500, "포인트 개별 지급에 실패했습니다.", request)
            
        return ex.ReturnOK(200, "포인트 개별 지급이 완료되었습니다.", request, {})
    
    elif assignSingleInput.save_type == "3" : # 개별회수

        point_sum = single_service.member_point_list(request, assignSingleInput)
        request.state.inspect = frame()
        ...
        if util.checkNumeric(res["spare_point"]) >= util.checkNumeric(assignSingleInput.saved_point) :
            # 회수할 포인트 금액이 충족되면 회수
            res = single_service.single_point_collect(request, assignSingleInput)
            request.state.inspect = frame()
            if res is None :
...
# be/manager/point/assign/single/xlsx/download
@router.post("/manager/point/assign/single/xlsx/download", dependencies=[Depends(api_same_origin)])
async def 복지포인트개별지급내역_엑셀_다운로드(
     request: Request
    ...
    if res != None :
        res = single_service.single_list(request, excelLogInput.params, True)
        request.state.inspect = frame()
