...
from app.service.manager.point import mall_service
...
def 복지몰포인트사용_필터조건 (request: Request):
...
# /be/manager/point/used/mall/init
@router.post("/manager/point/used/mall/init", dependencies=[Depends(api_same_origin)])
async def 복지몰포인트사용_내역_init (
     request: Request
    ...
    result.update({"filter": 복지몰포인트사용_필터조건(request)}) # 초기 필터

    return result

# /be/manager/point/used/mall/list
@router.post("/manager/point/used/mall/list", dependencies=[Depends(api_same_origin)])
async def 복지몰포인트사용_내역 (
     request: Request
    ...
    res = mall_service.mall_list(request, pPage_param)
    request.state.inspect = frame()

    return res


# be/manager/point/used/mall/xlsx/download
@router.post("/manager/point/used/mall/xlsx/download", dependencies=[Depends(api_same_origin)])
async def 복지포인트사용내역_엑셀_다운로드(
     request: Request
    ...
    if res != None :
        res = mall_service.mall_list(request, excelLogInput.params, True)
        request.state.inspect = frame()
