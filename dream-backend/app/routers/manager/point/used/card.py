...
from app.service.manager.point import used_service
...
def 복지카드사용_필터조건 (request: Request):
...
# /be/manager/point/used/card/init
@router.post("/manager/point/used/card/init", dependencies=[Depends(api_same_origin)])
async def 복지카드사용_내역_init (
     request: Request
    ,cardDeductInitInput: CardDeductInitInput
    ...
    params = {
        ...
        ,"page_total": 0
        ,"page_last": 0
        ,"use_type": cardDeductInitInput.use_type
        ,"filters": {
            "skeyword": '',
            "skeyword_type": '',
            "create_at": {
    ...
    result.update({"filter": 복지카드사용_필터조건(request)}) # 초기 필터

    return result

# /be/manager/point/used/card/list
@router.post("/manager/point/used/card/list", dependencies=[Depends(api_same_origin)])
async def 복지카드사용_내역 (
     request: Request
...
# be/manager/point/used/sikwon/card/xlsx/download
@router.post("/manager/point/used/sikwon/card/xlsx/download", dependencies=[Depends(api_same_origin)])
async def 식권카드사용_엑셀_다운로드(
     request: Request
    ...
    if res != None :
        dict_excelLogInput = jsonable_encoder(excelLogInput)
        cardDeductListInput = parse_obj_as(CardDeductListInput, dict_excelLogInput["params"])
        cardDeductListInput.use_type = "sikwon"

        res = used_service.deduct_list(request, cardDeductListInput, True)
        request.state.inspect = frame()
        ...
        worksheet.write("E1", "차감/소명 신청일", cell_format1)
        worksheet.write("F1", "포인트차감 승인일", cell_format1)
        worksheet.write("G1", "사용내역", cell_format1)
        worksheet.write("H1", "포인트 차감금액", cell_format1)
        worksheet.write("I1", "카드", cell_format1)
        worksheet.write("J1", "처리상태", cell_format1)
        worksheet.write("K1", "비고", cell_format1)
        # [ E ] 엑셀헤더
        ...
        for c in res["list"] :
            ...
            worksheet.write("G"+str(idx), c["detail"])
            worksheet.write("H"+str(idx), c["confirm_point"])
            worksheet.write("I"+str(idx), c["card"])
            worksheet.write("J"+str(idx), c["state"])
            worksheet.write("K"+str(idx), c["note"])
            idx = idx + 1
...
# be/manager/point/used/bokji/card/xlsx/download
@router.post("/manager/point/used/bokji/card/xlsx/download", dependencies=[Depends(api_same_origin)])
async def 복지카드사용_엑셀_다운로드(
     request: Request
    ,excelLogInput: ExcelLogInput
    ,user: TokenDataManager = Depends(get_current_active_manager)
):
    ...
    if res != None :
        dict_excelLogInput = jsonable_encoder(excelLogInput)
        cardDeductListInput = parse_obj_as(CardDeductListInput, dict_excelLogInput["params"])
        cardDeductListInput.use_type = "bokji"
