...
# 고객사 관리자 역할 전체 리스트
def admin_rols_list(request: Request, page_param: AdminRolesListInput):
    request.state.inspect = frame()
    db = request.state.db
    user = request.state.user

    where = ""
    where = where + f"WHERE site_id = '{page_param.site_id}' "
    where = where + f"AND partner_uid = '{page_param.partner_uid}' "
...
def admin_roles_read(request: Request, uid: int):
    ...
    sql = (
        db.query(
            ...
            ,T_ADMIN_ROLE.name
            ,T_ADMIN_ROLE.menus
            ,T_ADMIN_ROLE.partner_uid
            ,T_ADMIN_ROLE.site_id
        )
