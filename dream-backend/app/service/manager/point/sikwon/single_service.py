...
# 식권 포인트개별지급 - 리스트
def sikwon_single_list(request: Request, page_param: PPage_param, fullsearch:bool=False):
...
def sikwon_single_select_user_list(request: Request, assignSingleRead: AssignSingleRead):
    request.state.inspect = frame()
    db = request.state.db
    user = request.state.user
    ...
    point_stmt = (
        db.query(
             T_SIKWON.user_id
            ,func.sum(T_SIKWON.point).label('spare_point')
        )
...
# 포인트개별지급 - 식권포인트 등록
def single_sikwon_create(request: Request, assignSingleInput: AssignSingleInput):
    request.state.inspect = frame()
    db = request.state.db
    user = request.state.user
    ...
    for i in assignSingleInput.user_uids :
        ...
        create_log(request, db_item.uid, "T_SIKWON", "INSERT", "개별 식권포인트 지급", 0, db_item.uid, user.user_id)
        request.state.inspect = frame()

    return db_item

# 포인트개별지급 - 식권포인트 회수
def single_sikwon_point_return(request: Request, assignSingleInput: AssignSingleInput):
    request.state.inspect = frame()
    db = request.state.db
    user = request.state.user
    ...
    for i in assignSingleInput.user_uids :
        ...
        sql = db.query(T_SIKWON).filter(
             T_SIKWON.user_id.in_(stmt)
        ...
        for c in member_spare_point_res :
            if give_spare_point > 0 :
                ...
                if util.checkNumeric(c.point) > give_spare_point : 
                    ...
                    db.add(db_item)
                    c.point = c.point - give_spare_point # T_SIKWON UPDATE
                    give_spare_point = 0
        ...
        create_log(request, db_item.uid, "T_SIKWON", "INSERT", "개별 식권포인트 회수", 0, db_item.uid, user.user_id)
        request.state.inspect = frame()
        # [ E ] 차감한 금액만큼 예치금 전환
    return db_item
...
# 임직원 잔여 식권 포인트
def member_sikwon_point_list(request: Request, assignSingleInput: AssignSingleInput):
    request.state.inspect = frame()
    db = request.state.db
    user = request.state.user
    ...
    sql = (
        db.query(
             T_MEMBER.uid
            ,sikwon_point_stmt.c.spare_point
        )
        .join (
            sikwon_point_stmt, 
            T_MEMBER.user_id == sikwon_point_stmt.c.user_id,
            isouter = True # LEFT JOIN
        )
        .filter(*filters)
    )
