...
def order_list(request: Request, page_param : PPage_param):
    ...
    filters.append(getattr(T_B2B_ORDER, "token_name") == user.token_name)
    filters.append(getattr(T_B2B_ORDER, "sosok_uid") == user.partner_uid)
    ...
    if page_param.filters :
        if page_param.filters["skeyword"] :
            if page_param.filters["skeyword_type"] != "" :
                ...
                filters.append(
                    ...
                    | T_B2B_ORDER.apply_name.like("%"+page_param.filters["skeyword"]+"%")
                    | T_B2B_ORDER.apply_position.like("%"+page_param.filters["skeyword"]+"%")
                )
        ...
        if page_param.filters["category"] != '' :
            filters.append(T_B2B_ORDER.category == page_param.filters["category"])
