...
const PointLimitSikwon: NextPage = (props: any) => {
    const nav_id = 128;
    const crumbs = ['포인트 사용제한', '식권카드 허용 업종 조회'];
    const title_sub = '';
    const callout = [
        '식권카드 사용 건을 포인트로 차감신청 가능한 복지 허용 업종을 확인할 수 있습니다. ',
        '업종코드 및 표준산업분류코드는 홈텍스를 기준으로 합니다. ',
        '식권카드 허용 업종 수정요청은 공지/문의 > 문의하기에서 가능합니다.',
        '해당 페이지에서 조회되는 식권카드 허용 업종 리스트는 사용자 시스템의 포인트차감 > 소명신청 > 상단의  ‘식권카드 허용항목’ 버튼을 눌러 나오는 항목과 동일합니다.',
    ];
    ...
    useEffect(() => {
        props.response.params.filters.type = 'sikwon';
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/point/limit/sikwon/list`, p);
    ...
    const fnEditcode = async (mode: string) => {
        try {
            ...
            const { data } = await api.post(`/be/manager/point/limit/sikwon/edit`, edit_params);
    ...
    return (
        <>
            ...
            {excelModalOpen && (
                <ExcelModal setExcelModalOpen={setExcelModalOpen} params={params} url={'/be/manager/point/limit/card/xlsx/download'} title={'식권카드허용업종내역'} />
            )}
        </>
    );
};
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/point/limit/sikwon/init`, request);
        response = data;
    } catch (e: any) {
...
export default PointLimitSikwon;
