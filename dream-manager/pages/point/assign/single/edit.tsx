...
const SingleEdit: NextPage = (props: any) => {
    ...
    const crumbs = ['포인트 지급관리', '포인트 ' + (checkNumeric(router.query.save_type) > 2 ? '회수' : '지급')];
    const callout = [];
    const title_sub = '';
    ...
    const editing = async () => {
        try {
            ...
            const { data } = await api.post(`/be/manager/point/assign/single/edit`, params);
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/point/assign/single/read`, request);
        response = data;
    } catch (e: any) {
