...
const PointAssignList: NextPage = (props: any) => {
    const nav_id = 140;
    const crumbs = ['포인트 지급관리', '식권포인트 지급내역'];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/point/assign/sikwon`, p);
    ...
    return (
        <>
            ...
            {excelModalOpen && (
                <ExcelModal setExcelModalOpen={setExcelModalOpen} params={params} url={'/be/manager/point/assign/sikwon/xlsx/download'} title={'식권포인트지급내역'} />
            )}
        </>
    );
};
