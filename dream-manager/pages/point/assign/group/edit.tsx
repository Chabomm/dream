...
import { cls, checkNumeric, yyyymmdd_hhmmss, left, num2Cur } from '@/libs/utils';
import useForm from '@/components/form/useForm';

import { EditForm, EditFormTable, EditFormTH, EditFormTD, EditFormSubmit, EditFormCard, EditFormCardBody } from '@/components/UIcomponent/form/EditFormA';
...
const GroupEdit: NextPage = (props: any) => {
    const router = useRouter();
    const crumbs = ['포인트 지급관리', '포인트 지급 대량등록'];
    const callout = ['샘플엑셀파일에는 등록된 임직원의 정보가 자동으로 세팅되어 다운로드됩니다.', '샘플엑셀파일에서 임직원을 임의로 추가하거나 수정하지 마세요.'];
    const title_sub = '';
    ...
    const fnSampleExcelDown = async () => {
        ...
        try {
            await api({
                ...
                fileLink.setAttribute('download', yyyymmdd_hhmmss() + '_포인트지급_대량등록_샘플.xlsx');
        ...
        <LayoutPopup title={crumbs[crumbs.length - 1]} className="px-6 bg-slate-50">
            ...
            {bulkExcelModalOpen && (
                <BulkExcelModal
                    ...
                    setData={{ url: '/be/manager/point/assign/group/excel/upload/create' }}
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    setContext(ctx);
    var request: any = {
        save_type: checkNumeric(ctx.query.save_type),
    };
    var response: any = {};
    return {
