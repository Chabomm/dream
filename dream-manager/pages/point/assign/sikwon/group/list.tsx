...
import PointStatus from '@/pages/point/status';

const PointAssignSikwonList: NextPage = (props: any) => {
    const nav_id = 144;
    const router = useRouter();
    const crumbs = ['포인트 지급관리', '식권포인트 대량 지급'];
    const title_sub = ' 지급 사유 기준으로 식권포인트 지급 및 회수 내역을 확인하고, 포인트 대량지급을 할 수 있습니다.';
    const callout = [
        '지급 사유 기준으로 식권포인트 지급 및 회수 내역을 확인하고, 포인트 대량지급을 할 수 있습니다.',
    ...
    const fnSampleExcelDown = async () => {
        ...
        try {
            await api({
                ...
                fileLink.setAttribute('download', yyyymmdd_hhmmss() + '_식권포인트지급_대량등록_샘플.xlsx');
        ...
        <Layout nav_id={nav_id} crumbs={crumbs} title={crumbs[crumbs.length - 1]} user={props.user}>
            ...
            {bulkExcelModalOpen && (
                <BulkExcelModal
                    ...
                    setData={{ url: '/be/manager/point/assign/group/excel/upload/create', gubun: 'sikwon' }}
...
export default PointAssignSikwonList;
