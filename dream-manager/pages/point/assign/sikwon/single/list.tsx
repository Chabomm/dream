...
const PointAssignSikwonSingleList: NextPage = (props: any) => {
    const nav_id = 142;
    const crumbs = ['포인트 지급관리', '식권 포인트 개별 지급'];
    const title_sub = '식권포인트 지급 및 회수 내역을 확인할 수 있습니다.';
    const callout = [];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/point/assign/sikwon/single/list`, p);
    ...
    const goEdit = (save_type: number) => {
        ...
        var objPopup = window.open('', '개별식권포인트지급', 'width=1120, height=800, scrollbars=no, toolbar=no, status=no, resizable=no');
        newForm.target = '개별식권포인트지급'; // 타겟 : 위의 창띄우기의 창이름과 같아야 한다.
        newForm.action = `/point/assign/sikwon/single/edit?save_type=${save_type}`; // 액션경로
    ...
    return (
        <>
            ...
            {excelModalOpen && (
                <ExcelModal setExcelModalOpen={setExcelModalOpen} params={params} url={'/be/manager/point/assign/sikwon/single/xlsx/download'} title={'식권포인트개별지급내역'} />
            )}
        </>
    );
};
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/point/assign/sikwon/single/init`, request);
        response = data;
    } catch (e: any) {
...
export default PointAssignSikwonSingleList;
