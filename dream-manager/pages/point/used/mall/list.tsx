...
const PointUsedMallList: NextPage = (props: any) => {
    const nav_id = 124;
    const crumbs = ['포인트 사용관리', '복지포인트 사용내역'];
    const title_sub = '복지포인트 사용내역을 확인할 수 있습니다.';
    const callout = [
        '온라인 복지몰에서 복지포인트를 사용하여 결제한 내역을 확인할 수 있습니다.',
        '사용내역이 없는 경우에는 조회되지 않습니다.',
        '결제 시 사용한 복지포인트 금액은 검색결과>복지포인트 항목에 (-)로 표시됩니다.',
        '결제완료: 복지포인트 차감하여 결제 완료 상태.',
        '환불: 구매한 서비스 및 상품을 환불한 상태.',
        '포인트 지급/회수 내역은 복지포인트 지급내역에서 확인 가능합니다. ',
    ];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/point/used/mall/list`, p);
    ...
    return (
        <>
            ...
            {excelModalOpen && <ExcelModal setExcelModalOpen={setExcelModalOpen} params={params} url={'/be/manager/point/used/mall/xlsx/download'} title={'복지포인트사용내역'} />}
        </>
    );
};
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/point/used/mall/init`, request);
        response = data;
    } catch (e: any) {
...
export default PointUsedMallList;
