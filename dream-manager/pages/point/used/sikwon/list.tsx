...
const PointUsedSikwonList: NextPage = (props: any) => {
    const nav_id = 126;
    const crumbs = ['포인트 사용관리', '식권카드 사용내역'];
    const title_sub = '식권카드 사용내역을 확인할 수 있습니다.';
    const callout = [
        '식권카드를 사용내역 중 식권포인트로 차감완료/차감취소한 내역을 확인할 수 있습니다.',
        '사용내역이 없는 경우 조회되지 않습니다.',
        '소명승인 신청 건의 내역만 확인하고 싶은 경우에는 식권포인트관리 > 소명신청관리 > 소명승인에서 확인 가능합니다.',
        '온라인 식권몰에서 식권포인트로 바로 차감-결제한 내역은 조회되지 않습니다. ',
        '온라인 식권몰 사용내역만 확인하고 싶은 경우 식권포인트관리 > 식권포인트 사용관리 > 식권몰 포인트 사용내역에서 확인 가능합니다',
    ];
    ...
    return (
        <>
            ...
            <EditFormTable className="grid-cols-6 mb-5">
                ...
                <EditFormTH className="col-span-1">식권포인트 사용 합계</EditFormTH>
                <EditFormTD className="col-span-2">32,000원</EditFormTD>
            </EditFormTable> */}
            ...
            {excelModalOpen && (
                <ExcelModal setExcelModalOpen={setExcelModalOpen} params={params} url={'/be/manager/point/used/sikwon/card/xlsx/download'} title={'식권카드사용내역'} />
            )}
        </>
    );
};
...
export default PointUsedSikwonList;
