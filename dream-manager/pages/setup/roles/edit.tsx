...
const SetupRolesEdit: NextPage = (props: any) => {
    ...
    const editing = async mode => {
        try {
            ...
            s.values.mode = mode;
            const { data } = await api.post(`/be/manager/setup/roles/edit`, s.values);
            if (data.code == 200) {
            ...
            <LayoutPopup title={crumbs[crumbs.length - 1]}>
                ...
                <EditForm onSubmit={fn.handleSubmit}>
                    <EditFormTable className="grid-cols-6">
                        <EditFormTH className="col-span-1">역할명</EditFormTH>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/setup/roles/read`, request);
        response = data;
    } catch (e: any) {
