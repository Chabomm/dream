...
const SetupRolesList: NextPage = (props: any) => {
    const nav_id = 76;
    const crumbs = ['환경설정', '역할관리'];
    const title_sub = '관리자역할 내역 및 관리할 수 있습니다.';
    const callout = [];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/setup/roles/list`, p);
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/setup/roles/init`, request);
        response = data;
    } catch (e: any) {
