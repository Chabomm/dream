...
const AccountCateList: NextPage = (props: any) => {
    const nav_id = 137;
    const crumbs = ['정산', '카테고리별 복지몰 사용내역'];
    const title_sub = '';
    const callout = [];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/account/person/list`, p);
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/account/person/init`, request);
        response = data;
    } catch (e: any) {
...
export default AccountCateList;
