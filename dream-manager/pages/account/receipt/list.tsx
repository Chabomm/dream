...
const AccountReceiptList: NextPage = (props: any) => {
    const nav_id = 139;
    const crumbs = ['정산', '기타카드/현금영수증 사용내역'];
    const title_sub = '';
    const callout = [];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/account/receipt/list`, p);
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/account/receipt/init`, request);
        response = data;
    } catch (e: any) {
...
export default AccountReceiptList;
