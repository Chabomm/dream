...
const AccountMergeList: NextPage = (props: any) => {
    const nav_id = 133;
    const crumbs = ['정산', '전체 정산결과'];
    const title_sub = '';
    const callout = [];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/manager/account/merge/list`, p);
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/manager/account/merge/init`, request);
        response = data;
    } catch (e: any) {
...
export default AccountMergeList;
