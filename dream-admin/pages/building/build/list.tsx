...
const BuildingCounselList: NextPage = (props: any) => {
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/admin/building/build/list`, p);
    ...
    const goEdit = (item: any) => {
        window.open(`/building/build/edit?uid=${item.uid}`, '수기구축 수정', 'width=1120,height=800,location=no,status=no,scrollbars=yes');
    };

    const goBuild = (item: any) => {
        window.open(`http://localhost:10020/dream/build?uid=${item.uid}`, '수기구축 등록', 'width=1120,height=800,location=no,status=no,scrollbars=yes');
    };
    ...
    return (
        <Layout user={props.user} title="indendkorea admin console" nav_id={129} crumbs={['구축관리', '구축신청']}>
            ...
            <div className="col-table">
                ...
                {posts.map((v: any, i: number) => (
                    <div key={i} className="col-table-td grid grid-cols-11 bg-white transition duration-300 ease-in-out hover:bg-gray-100">
                        ...
                        <div className="">{v.staff_mobile}</div>
                        <div className="">{v.staff_email}</div>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/admin/building/build/init`, request);
        response = data;
    } catch (e: any) {
