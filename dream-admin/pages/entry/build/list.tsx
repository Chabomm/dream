...
const EntryBuildList: NextPage = (props: any) => {
    const nav_id = 129;
    const crumbs = ['구축관리', '구축신청'];
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/admin/entry/build/list`, p);
    ...
    const goEdit = (item: any) => {
        window.open(`/entry/build/edit?uid=${item.uid}`, '수기구축 수정', 'width=1120,height=800,location=no,status=no,scrollbars=yes');
    };
                ...
                <ListTableBody>
                    {posts?.map((v: any, i: number) => (
                        <tr key={`list-table-${i}`} className="">
                            ...
                            <td className="text-center">{v.staff_email}</td>
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/admin/entry/build/init`, request);
        response = data;
    } catch (e: any) {
