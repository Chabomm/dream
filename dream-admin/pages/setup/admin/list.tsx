...
const SetupAdminList: NextPage = (props: any) => {
    const nav_id = 82;
    ...
    const getPostsData = async p => {
        try {
            const { data } = await api.post(`/be/admin/setup/admin/list`, p);
    ...
    function openAdminUserEdit(uid: any) {
        window.open(`/setup/admin/edit?uid=${uid}`, '관리자 정보', 'width=1120,height=800,location=no,status=no,scrollbars=yes,left=400%,top=50%');
    }
...
export const getServerSideProps: GetServerSideProps = async ctx => {
    ...
    var response: any = {};
    try {
        const { data } = await api.post(`/be/admin/setup/admin/init`, request);
        response = data;
    } catch (e: any) {
